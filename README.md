# TorqueLab Service

Робочий прототип сайту автосервісу з онлайн-записом, базою SQLite, захистом від подвійного бронювання та email-підтвердженнями.

Інтерфейс використовує темну контрастну тему. Сервер приймає лише відомі послуги й тривалості, обмежує розмір та довжину полів запиту і не віддає базу даних або службові файли через HTTP.

## Запуск

```bash
python3 server.py
```

Відкрити: `http://127.0.0.1:8080`

## Email

Для реальної відправки задайте змінні середовища: `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `FROM_EMAIL`.

Без них бронювання працює й зберігається, але лист не відправляється. Дані записів зберігаються локально у `bookings.db`.

## Telegram

Для повідомлень адміністратору задайте секретні змінні `TELEGRAM_BOT_TOKEN` і `TELEGRAM_ADMIN_CHAT_ID`. Токен заборонено додавати у відкритий код або JavaScript.
